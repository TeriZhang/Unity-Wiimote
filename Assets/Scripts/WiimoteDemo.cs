...
public class WiimoteDemo : MonoBehaviour {
    ...
    public WaterSliderScript waterUI;
    public float totalWater;
    bool canGetWater;
    float waterRefilltimer, waterRefillTime;
	...
	void Update () {
        ...
        if (Input.GetKeyDown(KeyCode.Space))
        {
            if(canGetWater == true)
            {
                ...
                waterUI.remainingWater += 10;
                waterRefillTime = 0;
            }
        }

        if(waterRefillTime >= waterRefilltimer)
        {
            canGetWater = true;
        }
        else
        {
            waterRefillTime += 1 * Time.deltaTime;
            canGetWater = false;
        }
    }
